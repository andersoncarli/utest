...
`src/cache.js` inteiro, atrás de `TestCache(root)`. O cache não tem banco nem hash: vive nos
...
`src/scanner.js` é o único consumidor: os runners recebem o cache pronto de `scan()` e nunca
importam `src/cache.js`.

Coberto por `src/cache.t.js` (42 testes / 75 checks). Suite verde 2026-09-03.
