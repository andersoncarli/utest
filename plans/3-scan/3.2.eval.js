...
// frontmatter contra o `src/scanner.js` real, e fecha com a prova de campo do sprint
...
// onde `src/scanner.js` e `utest.js` são importáveis por caminho absoluto.
export default (t) => {
  ...
  t.real("3.2: um .t.js por CAMINHO num subdir fundo imprime o relatório completo (sprint 017)", async ({ sh, check, write }) => {
    ...
    // `src/scanner.js#makeFilter` (o mesmo `compileGlob` do walk).
