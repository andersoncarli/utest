...
`src/scanner.js` + `src/kinds.js`. A pipeline: walk por glob do `TEST.yaml` → separa testes de
fontes → `findTarget` pareia cada teste ao seu alvo → `scan()` devolve
`{ entries, uncovered, cache }`. `src/kinds.js` é o vocabulário de sufixos declarado UMA vez,
...
Coberto por `src/scanner.t.js` (19/26) e `src/kinds.t.js` (8/22). Suite verde 2026-09-03.
