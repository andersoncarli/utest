...
"<src/shims.js>"`. O arquivo já importa `check` explicitamente de `../src/check.js`
...
também são injetados — `src/setup.js:44` só lista `check`, então a expectativa é que os
...
divergirem. `utest/check.js#check(a, b, op, cb)` (`src/check.js:74-78`) é **declarativo**:
...
rodando `check()` fora do runner via script solto). Causa: `Check` (`src/check.js:70`) faz
`const t = boundTest || check.test` — `check.test` é setado GLOBALMENTE pelo runner antes
de cada teste (`utest.js:120`, `src/runner.js:61`), então qualquer `check()` chamado durante a
execução de um `test()` já se registra como assert daquele teste via `t.oncheck`
(`src/test.js:22-25`), **mesmo que o código nunca use o retorno**. Não há como uma chamada nua
...
é `checkFail(a, b)`: inverte o veredito internamente (`src/check.js:65-68`), então "isto DEVE
...
exatamente a condição que `src/cache.js#readPaired` (linha 223-231) exige para servir cache-hit).
...
sprint) — hipóteses não confirmadas: `depsFresh()` (src/cache.js) invalidando por alguma dep
...
Causa: `sweep()` roda `utest eval --json` com `cwd` = o projeto
...
- Ler o próprio `utest/check.t.js`/`src/test.t.js` (os testes do módulo, não a doc) revelou o
