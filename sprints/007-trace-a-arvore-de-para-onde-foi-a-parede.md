...
1. **`src/trace.js`** — cronômetro de REGIÕES de wall-time (o análogo de `probe`): `install`/
...
2. **`src/trace-preload.mjs`** — `bun --import` que um `.eval.js` splica; marca regiões DENTRO
...
4. **`clearTimeout` no `finally`** das corridas de `Promise.race` — `utest.js` e
   `src/runner.js`. Coberto por `src/leak.t.js`.
5. **`registerPhaseSetup`** movido para o `default` export de `src/kinds.js` + teste em
   `src/kinds.t.js`.

## Features que este sprint toca

- **5 profiling** — `src/trace.js`, `src/trace-preload.mjs`, `src/trace.t.js` (features 5.2, 5.3,
  5.4, 5.5).
- **1 core** — o `clearTimeout` (feature 1.3); `src/leak.t.js` +15.
- **3 scan** — `registerPhaseSetup` no default export + `src/kinds.t.js` (feature 3.3/3.4).

## Criterio de pronto

- `src/trace.t.js` verde (14 testes / 32 checks): região aninhada credita self ao pai,
  `(untracked)` = grand − Σ filhas, `wrapSpawns` restaura, `chromeTrace` em µs, enxerto de
  fragmento no tempo certo, splice sem env é no-op.
- `src/leak.t.js` verde: "timer do timeout é limpo quando o trabalho ganha a corrida".
- `src/kinds.t.js` verde: "registerPhaseSetup — um recurso que a fase monta 1×".
- `utest .` verde.

# REPORT

Primeiro sprint fechado sob o ZSS. src/trace.js + src/trace-preload.mjs + --trace: a árvore de para-onde-foi-a-parede; e o clearTimeout que fecha a fuga de 10s do event loop.

## O que aconteceu

- **`src/trace.js`** (novo, ~250 linhas) — `install`/`mark`/`end`/`region`, `wrapSpawns()`,
...
- **`src/trace-preload.mjs`** (novo, 35 linhas) — `globalThis.__uTrace` (region/mark), despeja
  `<UTEST_TRACE_OUT>.<pid>` JSON no exit, inerte sem o env.
- **`src/trace.t.js`** (novo, 14 testes / 32 checks) — cobre `src/trace.js` de ponta a ponta,
...
- **`clearTimeout` no `finally`** — `utest.js` (linha ~194) e `src/runner.js` (linhas ~27,
  ~90). `src/leak.t.js` +15 trava a mecânica.
- **`registerPhaseSetup`** no `default` export de `src/kinds.js` + teste em `src/kinds.t.js`.
- **Sidecar de falha no lugar certo** (`src/cache.js:278`) — `selfFile()` gravava em
  ...
  `src/leak.t.js` (o único `.t.js` sem alvo pareado, o que exercita esse caminho) agora
...
| feature | efeito |
|---|---|
| 5.2 trace | ✅ implementada + `src/trace.t.js` verde → 🟡 |
| 5.3 trace de subprocesso | ✅ `src/trace-preload.mjs` + enxerto, coberto → 🟡 |
| 5.4 trace.json | ✅ `chromeTrace`/`writeChromeTrace` cobertos → 🟡 |
| 5.5 roteamento --trace | ⚠️ implementado, sem `.t.js` do consumo → 🟠 |
| 1.3 sealed / clearTimeout | ✅ o `clearTimeout` fechado, `src/leak.t.js` cobre → 🟡 |
...
- **`src/trace.js` entrou com o melhor `.t.js` do repo** — 14 testes para um módulo novo, com
...
- **O consumo no `utest.js` NÃO tem teste** (feature 5.5, 🟠). O `if (provider) → trace,
  ...
  rodando o soml. `src/trace.js` está coberto; a cola que o liga ao CLI não.
...
- **`src/trace-preload.mjs` depende de o `.eval.js` do soml splicar `bun --import` na string
