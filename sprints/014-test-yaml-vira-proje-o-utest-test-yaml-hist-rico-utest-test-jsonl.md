...
`src/state.js` — `.utest/STATE`: histórico append-only de rodadas de scan sobre o `iodb`,
...
mão. `src/state.js` abre `.utest/STATE` (mesma infraestrutura do `src/ledger.js`, 8.1) e grava um
