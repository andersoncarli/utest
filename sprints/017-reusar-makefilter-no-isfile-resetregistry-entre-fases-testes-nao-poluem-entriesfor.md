...
1. **#1 — `utest path/x.t.js` num subdiretório não imprime relatório** (só
   ...
   pelo `makeFilter` que `src/scanner.js` já exporta e a fase de scan já usa.
...
2. **#3 — a fase `eval` não roda sob `utest .`, só quando nomeada.** Um *teste*
   ...
   **polui o singleton `ENTRY_PROVIDERS`** de `src/kinds.js`. Nas fases seguintes do mesmo
   ...
   reset). Dar a `src/kinds.js` um `resetRegistry()` e chamá-lo entre fases no utest.
...
`src/scanner.js` já resolve isto certo: `makeFilter(include, exclude)` (exportado, L53) compõe
...
`src/kinds.js`: `const ENTRY_PROVIDERS = new Map()` — estado de módulo, sem reset.
...
- **`src/scanner.js`**: garantir que `makeFilter` está exportado (já está, L53). Nenhuma mudança
...
- **`src/scanner.t.js`** (dentro de `test('findTarget: ...')` ou um `test()` novo
...
### Parte B — #3: `resetRegistry()` em `src/kinds.js`, chamado entre fases (feature 3.4)

- **`src/kinds.js`**: exportar `resetRegistry()` que limpa `ENTRY_PROVIDERS`, `EXECUTORS`,
...
- **`src/kinds.t.js`**: `test('resetRegistry: zera providers/executores/setups e restaura KINDS')`
...
- `utest src/scanner.t.js` verde (verify_tests de 3.2) — com o `belongs` novo coberto.
- `utest src/kinds.t.js` verde — com `resetRegistry` coberto.
...
double-path. `makeFilter` ja era exportado por `src/scanner.js` — so faltava o import em
`utest.js`.

**#3 — `resetRegistry()` em `src/kinds.js`, chamado entre fases** (`src/kinds.js` + `utest.js`, feature 3.4)
- `src/kinds.js`: `INITIAL_KINDS` guarda o vocabulario do import; `resetRegistry()` limpa
...
**Cobertura**
- `src/scanner.t.js` §"makeFilter": `**/*.t.js` casa em qualquer profundidade (raiz, 1, 2, fundo);
  um `.eval.js` nao casa `include` de `unit`; exclude vence include em qualquer nivel.
- `src/kinds.t.js` §"resetRegistry": registra kind + executor + entries + phaseSetup, afirma que
...
- `plans/3-scan/3.2.eval.js`: as seis regras de `findTarget` contra o `src/scanner.js` real; o
...
- `utest src/scanner.t.js` — verde (44 checks), verify_tests da 3.2
- `utest src/kinds.t.js` — verde (31 checks), verify_tests da 3.4
