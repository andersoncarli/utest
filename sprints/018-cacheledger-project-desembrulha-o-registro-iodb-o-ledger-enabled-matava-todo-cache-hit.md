...
`src/cacheLedger.js#project` (o redutor da projeção na abertura) lê `entry.event === 'run:start'`
...
`false`. E como `ledger?.enabled === true`, o `arbitrate` do `src/cache.js` (linha 466) rebaixa
**todo HIT de tempo válido** a MISS — "o cache de tempo dizia HIT, ledger discorda". O
mtime-carving está intacto (`src/scanner.t.js` mtime `…044` / alvo `src/scanner.js` `…000` / 44
...
1. **`src/cacheLedger.js`** — no laço da linha 112, desembrulhar o registro do iodb antes de
...
2. **`src/cacheLedger.t.js`** — teste de regressão sobre uma stream iodb REAL (não um mock de
...
- `utest src/cacheLedger.t.js` — verde
...
`src/cacheLedger.js#project` lia `entry.event` supondo eventos planos, mas o `iodb` os embrulha na
...
**`src/cacheLedger.js` — desembrulha o registro na leitura** (feature 2.7)
...
**`src/cacheLedger.t.js` — teste de regressão sobre a forma real do iodb**
...
`TestCache(root, { ledger: null }).read('src/scanner.t.js', 'src/scanner.js')` devolvia
`{ checks: 44 }` normalmente. O mtime-carving estava intacto (`src/scanner.t.js` mtime `…869044` /
alvo `src/scanner.js` `…869000` / 44 = a contagem). Era o ledger vetando.

`src/cacheLedger.js#project` (o redutor da projeção na abertura, linha 95) faz
...
`ledger?.enabled === true`, o `judge` do `arbitrate` (`src/cache.js:455`) é o ledger, que na
...
Por que os testes não pegaram: `src/cacheLedger.t.js` monta a stream com `fakeLedger([...])` de
...
`src/ledger.js#end` de fato grava; os testes usavam `test:result` solto).

## Prova

- `utest src/cacheLedger.t.js --force` — verde, 25 checks (era 22)
- `git stash` do fix + rodar `src/cacheLedger.t.js` → 2 ✘ no caso REGRESSÃO; `stash pop` → verde
